...
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, RecyclerView child, int layoutDirection) {
        ...
        if (lp.height == CoordinatorLayout.LayoutParams.MATCH_PARENT) {
            ...
            child.layout(0, 0, parent.getWidth(),
                    (int) (parent.getHeight() - getDependentViewCollapsedHeight()));
            return true;
        }
...
//        Log.i("info","dependentView.getTranslationY()="+dependentView.getTranslationY());
        float newTranslateY = dependentView.getTranslationY() - dy;
//        Log.i("info","newTranslateY="+newTranslateY);
        float minHeaderTranslate = -(dependentView.getHeight() - getDependentViewCollapsedHeight());
//        Log.i("info","minHeaderTranslate="+minHeaderTranslate);
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, RecyclerView child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        Log.i("info","dyUnconsumed="+dyUnconsumed);
        if (dyUnconsumed > 0) {
            return;
        }
    ...
    private boolean onUserStopDragging(float velocity) {
        ...
        if (Math.abs(velocity) <= 800) {
            Log.i("info","translateY="+translateY+"   translateY - minHeaderTranslate = "+(translateY - minHeaderTranslate));
        ...
        scroller.startScroll(0, (int) translateY, 0,
                (int) (targetTranslateY - translateY),
                (int) (1000000 / Math.abs(velocity)));
